...
require('lazy').setup({
  ...
  -- Allow search and insert of unicode 
  {
  ...
  -- asynchornous linking engine, for dotnet
  {
     'dense-analysis/ale'
  },
  -- enable debugging
...
--  Go back to folder view of root directory
...
-- use <space>tt to switch between nvim and terminal
...
-- start server with csproj or cs file only
g.OmniSharp_start_without_solution = 1
--  select selector ui [fzf | clap| ctrllp | unite]
g.OmniSharp_selector_ui = 'fzf'
--  select find usage ui [fzf | clap]
g.OmniSharp_selector_findusages = 'fzf'
-- use Omnisharp as highlight engine, even in insert mode
g.OmniSharp_highlighting = 3
-- show error id like CS8019 in error messages
g.OmniSharp_diagnostic_showid = 1
-- dotnet warning configuration
...
opt.scrolloff = 8
-- allocate the space for sign column
opt.signcolumn = "yes"
...
-- wget https://github.com/Samsung/netcoredbg/releases/download/2.2.3-992/netcoredbg-linux-amd64.tar.gz 
--  unpack in local bin folder
...
vim.keymap.set("n", "<F5>", require('dap').continue, { noremap = true, desc = "Launch the debugger" })
vim.keymap.set("n", "<leader><F5>", require('dap').close, { noremap = true, desc = "Stop debugger" })
vim.keymap.set("n", "<F9>", require('dap').toggle_breakpoint, {  noremap = true, desc = "Toggle breakpoint" })
vim.keymap.set("n", "<F10>", require('dap').step_over, {  noremap = true, desc = "Jump over function" })
vim.keymap.set("n", "<F11>", require('dap').step_into, {  noremap = true, desc = "Jump into function" })
vim.keymap.set("n", "<F12>", require('dap').step_out, {  noremap = true, desc = "Jump out of the function" })
...
-- Octo configuration
-- requires GH cli (steps for AlmaLinux)
-- sudo dnf install 'dnf-command(config-manager)'
-- sudo dnf config-manager --add-repo https://cli.github.com/packages/rpm/gh-cli.repo
-- sudo dnf install gh
-- call gh auth login and follow the steps
local octo = require("octo")
--  Possible parameter see  https://github.com/pwntester/octo.nvim#%EF%B8%8F-configuration 
octo.setup()
